...
def test_JB(size,popsize,generations,problem,mutation_type,mutation_prob,crossover_type,crossover_prob,selection_type,n_points,immigrant_insertion=None):
    ...
    ea(size)
    stats = ea.statistics()
    savedata_JB(stats,JB, popsize, generations,mutation_type, mutation_prob,crossover_type, crossover_prob, n_points, selection_type, immigrant_insertion)


def savedata_JB(stats,benchmark,size, popsize, generations,mutation_type, mutation_prob, crossover_type,crossover_prob, n_point, selection_type,immigrant_insertion):
...
def test_functions(popsize,generations,problem,mutation_type,mutation_prob,crossover_type,crossover_prob,selection_type,n_points,immigrant_insertion=None):
    ...
    for i in range(len(functions)):
        ...
        stats = ea.statistics()
        savedata(stats,functions[i][0], 20, popsize, generations,mutation_type, mutation_prob, crossover_type, crossover_prob, n_points, selection_type,immigrant_insertion)
                    ...
                    for mp in mutation_prob:
                        for ct in crossover_type:
                            for st in selection_type:
                                for _ in range(30):
                                        test_functions(popsize=dim_pop,
                                            generations=gen,
                                            problem=Function,
                                            mutation_type=mt,
                                            mutation_prob=mp,
